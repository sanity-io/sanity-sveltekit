 ...
 * @public
 */
export interface HandleVisualEditingConfig {
  /**
...
const setLocals = ({
  ...
  if (event.locals.sanity) {
    ...
    console.warn(
      'Sequence `handleVisualEditing` before `handleQueryLoader` or `handleLiveLoader` in `hooks.server.ts` to ensure the `event.locals.sanity` properties are set correctly'
    );
  }
 ...
 * @public
 */
export const handleVisualEditing = ({ client, preview }: HandleVisualEditingConfig): Handle => {
